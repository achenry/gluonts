...
def make_multivariate_dataset(
    ...
    grouper_train = Grouper(max_target_dim=dim, frequency=metadata.freq)
    grouper_test = Grouper(
        align_data=False, num_test_dates=num_test_dates, max_target_dim=dim, frequency=metadata.freq
    )
    return MultivariateDatasetInfo(
        dataset_name
