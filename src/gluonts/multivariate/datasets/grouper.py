...
class Grouper:
    ...
    def to_ts(self, data: DataEntry):
        return pd.Series(
            data['target'],
            index=pd.date_range(start=data['start'].to_timestamp(), periods=len(data["target"]), freq=data["start"].freq), # CHANGE
        )
    ...
    def _align_data_entry(self, data: DataEntry) -> DataEntry:
        ...
        ts = self.to_ts(data)
        d['target'] = ts.reindex(
            pd.date_range(start=self.first_timestamp, end=self.last_timestamp, freq=d["start"].freq), #CHANGE
            fill_value=ts.mean(),
        )
    ...
    def _prepare_test_data(self, dataset):
        ...
        def left_pad_data(data: DataEntry):
            ...
            filled_ts = ts.reindex(
                # pd.date_range(start=self.first_timestamp, end=ts.index[-1], freq=data['start'].freq),
                pd.date_range(start=self.first_timestamp, end=self.last_timestamp, freq=data['start'].freq),
                fill_value=0.0,
            )
